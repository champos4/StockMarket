...
#include <stdio.h>
 ...
 float find_max_tab(int size, float tab[size]);
 int fill_int_tab(float value, int size, float stock_prices[], float interesting_stock[]);
...
int main(){
   ...
   ni_stock= fill_int_tab(max_stock/2, 10, stock_prices, interesting_stock);
   ni_cds= fill_int_tab(max_cds/2, 11, cds_prices , interesting_cds);
   ...
   printf("Max Price:: %4.2f \n",max_deriv);
   print_tab_with_max(max_deriv, 10, deriv_prices);
 ...
 int fill_int_tab(float value, int size, float tab[], float int_tab[])
 {
  ...
  void high_low(int *high, int *low, int row, int r, int c, float table[r][c])
  {
      ...
      int lowValue=0;
      int j;
      float closingValue = table[row][c-1];

      for(j=0;j<c-1;j++)
      {
          if(table[row][j] > closingValue)
            highvalue ++;
          if(table[row][j]<closingValue)
            lowValue ++;
      }
